...
我们将构建一个服务器，提供两个工具：`get-alerts` 和 `get-forecast`。然后我们将服务器连接到 MCP 宿主应用（在本例中是 Claude for Desktop）：
...
描述: {props.get('description', '没有可用的描述')}
指示: {props.get('instruction', '没有提供具体指示')}
"""
```
...
您的服务器已经完成！运行 `uv run weather.py` 来确认一切都正常工作
...
首先，确保您已经安装了 Claude for Desktop。[您可以在这里安装最新版本](https://claude.ai/download)。如果您已经安装了 Claude for Desktop，**请确保它已更新到最新版本**

我们需要为您想要使用的任何 MCP 服务器配置 Claude for Desktop。为此，请在文本编辑器中打开您的 Claude for Desktop 应用程序配置文件，路径为 `~/Library/Application Support/Claude/claude_desktop_config.json`。如果该文件不存在，请确保创建它
...
然后，您将在 `mcpServers` 键中添加您的服务器。MCP 用户界面元素只有在至少正确配置了一个服务器的情况下才会在 Claude for Desktop 中显示
...
2. 通过运行 `uv --directory /绝对/路径/到/父/文件夹/weather run weather` 来启动它
    ...
    if (!pointsData) {
      return {
        content: [
          {
            type: "text",
            text: `无法获取坐标的网格点数据：${latitude}, ${longitude}。该位置可能不受 NWS API 支持（仅支持美国位置）`,
          },
        ],
      };
    }
...
```json Node
{
    "mcpServers": {
        "weather": {
            "command": "node",
            "args": [
                "/绝对/路径/到/父/文件夹/weather/build/index.js"
            ]
        }
    }
}
...
```json Node
{
    "mcpServers": {
        "weather": {
            "command": "node",
            "args": [
                "C:\\路径\\到\\父\\文件夹\\weather\\build\\index.js"
            ]
        }
    }
}
...
这告诉 Claude for Desktop：
1. 有一个名为 "weather" 的 MCP 服务器
2. 通过运行 `node /绝对/路径/到/父/文件夹/weather/build/index.js` 来启动它
...
有关更多信息，请参阅 [MCP 服务器引导启动器](https://docs.spring.io/spring-ai/reference/api/mcp/mcp-server-boot-starter-docs.html) 参考文档
...
首先，确保您已经安装了 Claude for Desktop
    ...
    "mcpServers": {
      "spring-ai-mcp-weather": {
        ...
        "args": [
          ...
          "-jar",
          "/绝对/路径/到/父/文件夹/mcp-weather-stdio-server-0.0.1-SNAPSHOT.jar"
        ]
      }
    }
  }
...
2. 通过运行 `java -jar /绝对/路径/到/父/文件夹/mcp-weather-stdio-server-0.0.1-SNAPSHOT.jar` 来启动它
...
var stdioParams = ServerParameters.builder("java")
  .args("-jar", "/绝对/路径/到/父/文件夹/mcp-weather-stdio-server-0.0.1-SNAPSHOT.jar")
  .build();
...
创建一个使用 `spring-ai-mcp-client-spring-boot-starter` 依赖的新引导启动器应用程序：
...
让我们确保 Claude for Desktop 正在检测到我们在 `weather` 服务器中公开的两个工具。您可以通过查找锤子 <img src="/images/claude-desktop-mcp-hammer-icon.svg" style={{display: 'inline', margin: 0, height: '1.3em'}} /> 图标来做到这一点：
...
如果您的服务器没有被 Claude for Desktop 检测到，请前往[故障排除](#troubleshooting)部分获取调试提示
...
这不是错误 - 它只是意味着该州目前没有天气警报。尝试其他州或在恶劣天气期间检查
</Accordion>
...
<CardGroup cols={2}>
  ...
  >
    了解如何使用像 Claude 这样的大语言模型来加速您的 MCP 开发
  </Card>
</CardGroup>
