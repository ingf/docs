...
[您可以在这里安装最新版本。](https://claude.ai/download) 如果您已经有 Claude for Desktop，请**确保它已更新到最新版本**。

我们需要为您想要使用的 MCP 服务器配置 Claude for Desktop。
为此，请在文本编辑器中打开 `~/Library/Application Support/Claude/claude_desktop_config.json` 中的 Claude for Desktop 应用程序配置。
如果文件不存在，请确保创建它。
...
只有在至少一个服务器配置正确时，MCP UI 元素才会在 Claude for Desktop 中显示。
